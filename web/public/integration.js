...
(function() {
    ...
    const AccessPDF = {
        ...
        init: function(userConfig = {}) {
            ...
            if (!this.config.integrationId) {
                console.warn('AccessPDF: integrationId not provided. Some features may not work.');
            }
        ...
        addAccessibilityBadge: function(link) {
            ...
            `;
            badge.title = 'This PDF has been processed for accessibility and is searchable by AI assistants like ChatGPT, Claude, and Gemini';
        ...
        addDiscoveryMetadata: function() {
            ...
            const metaTags = [
                ...
                { name: 'accesspdf-api-endpoint', content: `${this.apiBase}/public/embeddings/search` },
                { name: 'accessible-documents-count', content: pdfCount.toString() },
                { name: 'accessibility-compliance', content: 'wcag-2.1-aa,pdf-ua-1,section-508' },
        ...
        addDocumentStructuredData: function(link, accesspdfId) {
            ...
            const existingScript = document.querySelector(`script[data-accesspdf-id="${accesspdfId}"]`);
            if (!existingScript) {
                ...
                mutations.forEach((mutation) => {
                    mutation.addedNodes.forEach((node) => {
                        if (node.nodeType === 1) { // Element node
                            ...
                            const pdfLinks = node.querySelectorAll && node.querySelectorAll('a[href$=".pdf"], a[href*=".pdf?"], a[href*=".pdf#"]');
                            if (pdfLinks) {
                                pdfLinks.forEach(link => {
                                    if (this.enhancePDFLink(link)) {
                                        foundNew = true;
        ...
        generatePotentialId: function(pdfUrl) {
            ...
            return btoa(this.config.domain + ':' + path).replace(/[^a-zA-Z0-9]/g, '').substring(0, 16);
        },
    ...
    console.log('AccessPDF Integration v' + AccessPDF.version + ' loaded successfully');
