...
class APIKeyRepository(BaseRepository):
    ...
    def validate_api_key(self, raw_key: str) -> Optional[APIKey]:
        ...
        try:
            doc = self.collection.find_one({"key_hash": key_hash, "is_active": True})
    ...
    def update_api_key(self, key_id: str, updates: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            result = self.collection.update_one({"_id": key_id}, {"$set": updates})
            return result.modified_count > 0
        except Exception as e:
...
def get_api_key_repository() -> APIKeyRepository:
    ...
    if api_key_repo is None:
        connection_string = os.getenv(
            "MONGODB_URI", "mongodb://localhost:27017/pdf_accessibility"
        )
