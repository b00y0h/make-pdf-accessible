...
class MongoConnection:
    ...
    def _connect_with_retry(self, max_retries: int = 3, retry_delay: float = 1.0):
        ...
        for attempt in range(max_retries + 1):
            try:
                logger.info(
                    f"Attempting to connect to MongoDB (attempt {attempt + 1}/{max_retries + 1})"
                )
                ...
                logger.info(
                    f"Successfully connected to MongoDB database: {self._database_name}"
                )
                return
                ...
                if attempt < max_retries:
                    ...
                    raise MongoConnectionError(
                        f"Failed to connect to MongoDB after {max_retries + 1} attempts: {e}"
                    )
