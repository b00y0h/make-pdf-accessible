...
class AltTextRepository(BaseRepository):
    ...
    def create_document_alt_text(self, doc_id: str, figures_data: List[dict]) -> dict:
        ...
        try:
            ...
            doc_data = {
                ...
                'pending_review': len([f for f in figures if f['status'] == 'needs_review']),
                'approved': 0,
                'edited': 0,
                'created_at': now,
                'updated_at': now
        ...
        try:
            ...
            for i, figure in enumerate(doc.get('figures', [])):
                if figure['figure_id'] == figure_id:
                    figure_index = i
                    next_version = max([v.get('version', 0) for v in figure.get('versions', [])]) + 1
                    break
            ...
            update_doc = {
                '$push': {
                    f'figures.{figure_index}.versions': new_version
                },
                '$set': {
                    f'figures.{figure_index}.current_version': next_version,
                    f'figures.{figure_index}.status': new_status,
                    f'figures.{figure_index}.approved_text': new_text if new_status == 'approved' else current_figure.get('approved_text'),
                    'updated_at': now
                }
            }
            ...
            logger.error(f"Error editing alt text for figure {figure_id} in document {doc_id}: {e}")
            return False
    ...
    def update_figure_status(
        ...
        try:
            ...
            if status == 'approved':
                ...
                if current_text:
                    update_fields[f'figures.{figure_index}.approved_text'] = current_text

                # Add approval version if there was a comment
                if comment:
                    next_version = max([v.get('version', 0) for v in current_figure.get('versions', [])]) + 1
                    approval_version = {
                        'version': next_version,
                        'text': current_text,
                        'editor_id': editor_id,
                    ...
                    self.update_one(
                        {'docId': doc_id},
                        {
                            '$push': {f'figures.{figure_index}.versions': approval_version},
                            '$set': {f'figures.{figure_index}.current_version': next_version}
                        }
                    )
    ...
    def bulk_update_status(
        ...
        try:
            ...
            for figure_id in figure_ids:
                if self.update_figure_status(doc_id, figure_id, status, editor_id, editor_name, comment):
                    updated_count += 1
    ...
    def get_figure_history(self, doc_id: str, figure_id: str) -> Optional[dict]:
        ...
        try:
            doc = self.find_one(
                {'docId': doc_id, 'figures.figure_id': figure_id},
                {'figures.$': 1}
            )
            ...
            if doc and 'figures' in doc and doc['figures']:
                ...
                return {
                    'figure_id': figure_id,
                    'versions': sorted(figure.get('versions', []), key=lambda x: x.get('version', 0)),
    ...
    def revert_to_version(
        ...
        try:
            ...
            logger.error(f"Error reverting figure {figure_id} to version {version}: {e}")
            return False
