...
async def get_document_alt_text(
    document_id: UUID,
    status_filter: Optional[AltTextStatus] = Query(None, description="Filter by figure status"),
...
async def edit_figure_alt_text(
    ...
    try:
        ...
        logger.error(f"Failed to edit alt text for figure {edit_request.figure_id}: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def update_figure_status(
    ...
    try:
        ...
        metrics.add_metric(name="AltTextStatusUpdates", unit="Count", value=updated_count)
