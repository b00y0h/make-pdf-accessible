...
class ReportsSummaryResponse(BaseModel):
    ...
    completed_documents: int = Field(..., description="Successfully completed documents")
    processing_documents: int = Field(..., description="Documents currently being processed")
    ...
    weekly_stats: List[Dict[str, Any]] = Field(..., description="Weekly statistics with aggregated data")
...
class AltTextFigure(BaseModel):
    ...
    confidence: Optional[float] = Field(None, ge=0, le=1, description="AI confidence score")
    generation_method: Optional[str] = Field(None, description="How alt text was generated")
    versions: List[AltTextVersion] = Field(default_factory=list, description="Version history")
    context: Optional[Dict[str, Any]] = Field(default_factory=dict, description="Figure context metadata")
    bounding_box: Optional[Dict[str, float]] = Field(None, description="Figure location")
...
class AltTextDocumentResponse(BaseModel):
    ...
    doc_id: UUID = Field(..., description="Document ID")
    figures: List[AltTextFigure] = Field(default_factory=list, description="Figures with alt text")
...
class AltTextEditRequest(BaseModel):
    ...
    comment: Optional[str] = Field(None, max_length=200, description="Comment about the change")
...
class AltTextBulkStatusRequest(BaseModel):
    ...
    comment: Optional[str] = Field(None, max_length=200, description="Comment for bulk action")
