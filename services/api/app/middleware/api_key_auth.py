...
from services.shared.mongo.api_keys import get_api_key_repository, APIKey
...
class RateLimiter:
    ...
    def _cleanup_old_requests(self, now: float):
        ...
        for key_id in list(self.requests.keys()):
            self.requests[key_id] = [req_time for req_time in self.requests[key_id] if req_time > cutoff]
...
class APIKeyAuthMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        try:
            ...
            # Check rate limiting
            if key_obj.rate_limit and not rate_limiter.is_allowed(key_obj.id, key_obj.rate_limit):
                raise HTTPException(
