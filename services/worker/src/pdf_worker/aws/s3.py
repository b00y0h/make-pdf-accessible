...
class S3Client:
    ...
    def __init__(self, region_name: str | None = None) -> None:
        ...
        try:
            self._client = boto3.client(
                "s3", region_name=region_name or config.aws_region
            )
        ...
        logger.info(
            f"Initialized S3 client for region: {region_name or config.aws_region}"
        )
    ...
    def upload_file(
        ...
        try:
            ...
            raise S3Error(
                f"Failed to upload file to S3: {error_code}", bucket=bucket, key=key
            ) from e
    ...
    def upload_bytes(
        ...
        try:
            ...
            error_code = e.response.get("Error", {}).get("Code", "Unknown")
            raise S3Error(
                f"Failed to upload bytes to S3: {error_code}", bucket=bucket, key=key
            ) from e
    ...
    def upload_json(
        ...
        json_metadata = {
            "data_type": "json",
            "records_count": str(
                len(data.get("elements", [])) if "elements" in data else 0
            ),
        }
        if metadata:
    ...
    def download_file(self, bucket: str, key: str, local_path: str | Path) -> Path:
        ...
        try:
            ...
            if error_code == "NoSuchKey":
                raise S3Error(
                    f"Object not found: s3://{bucket}/{key}", bucket=bucket, key=key
                )

            raise S3Error(
                f"Failed to download from S3: {error_code}", bucket=bucket, key=key
            ) from e
    ...
    def download_bytes(self, bucket: str, key: str) -> bytes:
        ...
        try:
            ...
            logger.info(
                f"Successfully downloaded {len(content)} bytes from s3://{bucket}/{key}"
            )
            return content
    ...
    def download_json(self, bucket: str, key: str) -> dict[str, Any]:
        ...
        try:
            ...
            raise S3Error(
                f"Failed to parse JSON from S3 object: {e}", bucket=bucket, key=key
            ) from e
    ...
    def object_exists(self, bucket: str, key: str) -> bool:
        ...
        try:
            ...
            if e.response.get("Error", {}).get("Code") == "404":
                return False
            raise S3Error(
                f"Failed to check object existence: {e}", bucket=bucket, key=key
            ) from e
    ...
    def get_object_metadata(self, bucket: str, key: str) -> dict[str, Any]:
        ...
        try:
            ...
            raise S3Error(
                f"Failed to get object metadata: {error_code}", bucket=bucket, key=key
            ) from e
    ...
    def copy_object(
        ...
        try:
            ...
            logger.info(
                f"Successfully copied s3://{source_bucket}/{source_key} to {dest_uri}"
            )
    ...
    def delete_object(self, bucket: str, key: str) -> None:
        ...
        try:
            ...
            if error_code != "NoSuchKey":
                raise S3Error(
                    f"Failed to delete S3 object: {error_code}", bucket=bucket, key=key
                ) from e
    ...
    def list_objects(
        ...
        try:
            ...
            error_code = e.response.get("Error", {}).get("Code", "Unknown")
            raise S3Error(
                f"Failed to list S3 objects: {error_code}", bucket=bucket, key=prefix
            ) from e
