...
class SQSMessage:
    ...
    def from_sqs_record(cls, record: SQSRecord) -> "SQSMessage":
        ...
        try:
            # Parse JSON body
            body = (
                json.loads(record.body) if isinstance(record.body, str) else record.body
            )
...
class SQSClient:
    ...
    def __init__(self, region_name: str | None = None) -> None:
        ...
        try:
            self._client = boto3.client(
                "sqs", region_name=region_name or config.aws_region
            )
        except Exception as e:
            raise WorkerConfigError(f"Failed to initialize SQS client: {e}")

        logger.info(
            f"Initialized SQS client for region: {region_name or config.aws_region}"
        )
    ...
    def send_message(
        ...
        try:
            ...
            raise SQSError(
                f"Failed to send message to SQS: {error_code}", queue_url=queue_url
            ) from e
    ...
    def send_batch_messages(
        ...
        try:
            ...
            for i, message in enumerate(messages):
                ...
                if message_group_id:
                    ...
                    entry["MessageDeduplicationId"] = (
                        f"{message_group_id}-{i}-{int(datetime.utcnow().timestamp())}"
                    )
            ...
            logger.info(
                f"Batch send completed: {successful_count} successful, {failed_count} failed"
            )
    ...
    def delete_batch_messages(
        ...
        try:
            ...
            logger.info(
                f"Batch delete completed: {successful_count} successful, {failed_count} failed"
            )
    ...
    def get_queue_attributes(self, queue_url: str) -> dict[str, Any]:
        ...
        try:
            ...
            error_code = e.response.get("Error", {}).get("Code", "Unknown")
            raise SQSError(
                f"Failed to get queue attributes: {error_code}", queue_url=queue_url
            ) from e
...
class SQSMessageProcessor:
    ...
    def process_sqs_event(
        self, event: SQSEvent, message_handler: Callable[[SQSMessage], dict[str, Any]]
    ) -> list[dict[str, Any]]:
    ...
    @tracer.capture_method
    def send_to_dlq(
        self, dlq_url: str, original_message: SQSMessage, error_info: dict[str, Any]
    ) -> None:
        ...
        try:
            ...
            self.sqs_client.send_message(
                ...
                message_attributes={
                    ...
                    "original_queue": original_message.attributes.get(
    ...
    def create_process_message(
        self, doc_id: str, step: str, input_data: dict[str, Any], priority: bool = False
    ) -> dict[str, Any]:
