...
class DocumentElement(BaseModel):
    ...
    bounding_box: Optional[BoundingBox] = Field(
        None, description="Element bounding box"
    )
...
class ListItem(DocumentElement):
    ...
    marker: Optional[str] = Field(
        None, description="List marker (bullet, number, etc.)"
    )
...
class StructureRequest(BaseModel):
    ...
    textract_s3_key: Optional[str] = Field(
        None, description="S3 key for Textract results"
    )
...
class StructureResult(BaseModel):
    ...
    document_json_s3_key: Optional[str] = Field(
        None, description="S3 key for structured document JSON"
    )
