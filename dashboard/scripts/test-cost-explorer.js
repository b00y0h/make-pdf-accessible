...
async function testCostExplorerAccess() {
  ...
  try {
    ...
    if (response.ResultsByTime && response.ResultsByTime.length > 0) {
      const latestPeriod =
        response.ResultsByTime[response.ResultsByTime.length - 1];
      console.log(
    ...
    console.log(
      `✅ GetDimensionValues successful! Found ${dimensionResponse.DimensionValues?.length || 0} services`
    );
  } catch (error) {
    ...
    if (error.name === 'AccessDenied') {
      ...
      console.error(
        '1. Verify IAM permissions include ce:GetCostAndUsage and ce:GetDimensionValues'
      );
