...
const AVAILABLE_PERMISSIONS = [
  ...
  { id: 'documents.process', label: 'Process Documents', category: 'Documents' },
...
export function APIKeyManager() {
  ...
  const [createForm, setCreateForm] = useState<CreateAPIKeyRequest>({
    name: '',
    permissions: ['documents.read', 'documents.upload', 'documents.process', 'reports.read'],
  ...
  const loadUsageStats = async () => {
    try {
      const headers = await getAuthHeaders()
      const response = await fetch(`${API_BASE_URL}/api-keys/usage-stats`, { headers })
    ...
    <div className="space-y-6">
      ...
      {usageStats && (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <Card>
            <CardContent className="p-4">
              <div className="text-2xl font-bold text-green-600">{usageStats.active_keys}</div>
              <p className="text-sm text-muted-foreground">Active Keys</p>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-4">
              <div className="text-2xl font-bold">{usageStats.total_usage.toLocaleString()}</div>
