...
export function useDocumentPolling(
  ...
  const isFailed = document?.status === 'failed' || document?.status === 'validation_failed' || document?.status === 'notification_failed'
  ...
  }, [document?.status, docId, shouldPoll, pollingInterval])
