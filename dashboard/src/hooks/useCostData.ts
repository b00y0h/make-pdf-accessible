  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const timeseriesResponse = await fetch(
        `${basePath}/timeseries?${timeseriesParams}`
      );
      ...
      if (dataSource === 'ce' && !timeseriesData.ok) {
        throw new Error(
          timeseriesData.error || 'Failed to fetch timeseries data'
        );
      }
      ...
      if (dataSource === 'athena') {
        ...
        const summaryResponse = await fetch(
          `${basePath}/services?${summaryParams}`
        );
        ...
        const summaryResponse = await fetch(
          `${basePath}/summary?${summaryParams}`
        );
      ...
      if (dataSource === 'ce') {
        ...
        const servicesResponse = await fetch(
          `${basePath}/services?${servicesParams}`
        );
        ...
        if (summaryData?.services) {
          availableServices = summaryData.services.map(
      ...
      if (dataSource === 'ce') {
        ...
        timeseriesPoints = (timeseriesData.dataPoints || []).map(
          (point: any) => ({
            time: point.month,
            amount: point.amount,
...
export function useForecastData(filters: Pick<CostFilters, 'metric'>) {
  ...
  const fetchForecast = useCallback(async () => {
    try {
      ...
      params.set(
        'metric',
        filters.metric === 'UnblendedCost' ? 'UNBLENDED_COST' : 'BLENDED_COST'
      );
